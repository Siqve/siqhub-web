...
export const getDB = () => {
    return {
        color: () => {
            return actionFunctions<ColorDB, ColorInsertDB, ColorUpdateDB>(
        ...
        device: () => {
            return actionFunctions<Device, DeviceInsertDB, DeviceUpdateDB>(
        ...
        custom: <T, T_RETURN, T_UPDATE>(tableName: string, column?: string, query?: string) => {
            return actionFunctions<T, T_RETURN, T_UPDATE>(tableName, column, query);
        },
        admin() {
